...
async def handle_timeout_loss(game_id: UUID, loser_color: PlayerColor, active_game: ActiveGame, db: AsyncSession):
    winner = (
        active_game.black_id if loser_color == PlayerColor.white
        else active_game.white_id
    )

    active_game.status = "timeout"
    await save_active_game(active_game)

    # Guardar resultado en DB
    game = await db.get(Game, game_id)
    game.status = GameStatus.completed
    game.result = GameResult.white_win if loser_color == PlayerColor.black else GameResult.black_win
