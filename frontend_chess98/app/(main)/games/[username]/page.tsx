  ...
  useEffect(() => {
    const fetchGames = async () => {
      ...
      try {
        ...
        const response = await gameService.getGameByUsername(
          username,
          apiPage,
          gamesPerApiPage
        );
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      result = result.filter((game) =>
        game.opponent.username.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    ...
    if (timeControlFilter !== "all") {
      result = result.filter(
        (game) => game.time_control_str === timeControlFilter
      );
    }
    ...
    result.sort((a, b) => {
      if (sortBy === "date") {
        ...
        return sortOrder === "desc"
          ...
          : a.opponent.rating - b.opponent.rating;
                  ...
                  <div className="mt-8 border-t border-slate-700 pt-6">
                    ...
                    <Pagination>
                      <PaginationContent>
                        ...
                        {Array.from(
                          { length: Math.min(5, totalApiPages) },
                          (_, i) => {
                            // Show first page, last page, current page, and pages around current
                            let pageToShow;
                            if (totalApiPages <= 5) {
